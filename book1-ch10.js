...
class Camera {
	constructor(lookFrom, lookAt, vup, vfov, aspect) {
		// The viewport is 1 unit in front of the origin (lookFrom)
		...
		const su = scaleVec(half_width, u);
		const sv = scaleVec(half_height, v);
		this.origin = lookFrom;
		this.lower_left_corner = subVec(lookFrom, addVec(w,addVec(su,sv)));
		this.horizontal = scaleVec(2, su);
		this.vertical = scaleVec(2, sv);
	}
	getRay(u, v) {
		// p is a point on the viewport plane
		const p = addVec(addVec(scaleVec(u, this.horizontal), scaleVec(v, this.vertical)), this.lower_left_corner);

		return new Ray(this.origin, subVec(p, this.origin));
	}
}
...
function main() {
	...
	// Camera
	const camera = new Camera([-2,2,1],[0,0,-1],[0,1,0],30, cv_lx / cv_ly);
